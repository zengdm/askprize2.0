...
Page({
  ...
  cb_answer: function (res, opt) {
    ...
    if (res.code == 0) {
      ...
      if (res.data.pass.code !='0') {
       //用户正在答题中 问答活动是否已结束
        that.data.stop = true;
        wx.showModal({
          title: '',
          content: res.data.pass.message,
          showCancel: false,
  ...
  _showResult: function () {
    var that = this;
    console.log('用户正在答题中 问答活动是否已结束',that.data.stop);
    //用户正在答题中 问答活动是否已结束
    if(that.data.stop){
      return false
    }
    ...
    console.log('回答正确与否',that.data.subAnswer.isright)
    ...
    console.log('选中选项的颜色变化--',that.data.onColor)
    that.setData({
      onColor: that.data.onColor,
    });
  },
